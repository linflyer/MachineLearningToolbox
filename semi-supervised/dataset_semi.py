...
class DatasetProvider:
  ...
  def __init__(self, path):
    ...
    for line in open(path):
      conf, label, text = line.strip().split('|')
  ...
  def load(self, path, maxlen=float('inf')):
    ...
    for line in open(path):
      ...
      if(float(conf) == 1):
        lab_examples.append(example)
        labels.append(self.label2int[label])
      else:
        un_examples.append(example)

    return lab_examples, labels, un_examples

  def loadOne(self, path, maxlen=float('inf')):
    """Convert sentences (examples) into lists of indices"""
